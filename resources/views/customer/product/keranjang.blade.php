                    ...
                    <table class="w-full max-w-[933px] ">
                        ...
                        <tbody>
                            <tr class="border-b border-gray-4 item-keranjang">
                                <td class="p-2 text-gray-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <p class="font-md">Monstera</p>
                                    </div>
                                </td>
                                <td class="p-2 text-gray-2 harga" data-harga="10000"> Rp. 10.000</td>
                                <td class=" text-gray-2 ">
                                    ...
                                    <div class="flex gap-2 items-center my-[24px]">
                                        <div class="mt-2  items-center py-2 px-1 rounded-[8px] w-[134px] flex justify-between"
                                            style="box-shadow: 1px 1px 7px rgba(0, 0, 0, 0.25);">
                                            <button type="button"
                                                class="mi text-gray-3 cursor-pointer hover:opacity-80 tblPlusHari">add</button>
                                            <input type="number" readonly
                                                class="focus:outline-none w-[60px] text-center font-xl-responsive hide-arrow hari-input"
                                                x-bind:value="durasi" value="0" />
                                            <button type="button"
                                                class="mi text-gray-3 cursor-pointer hover:opacity-80 tblMinHari">remove</button>
                                        </div>
                                        <div>Hari</div>
                                    </div>
                                </td>
                                <td class="p-2 text-gray-2">Rp <span class="subtotal" data-subtotal="0">0</span>
                                </td>
                                <td class="p-2 text-gray-2">
                            ...
                            <tr class="border-b border-gray-4 item-keranjang">
                                ...
                                <td class="p-2 text-gray-2 harga" data-harga="5000"> Rp. 5.000</td>
                                <td class=" text-gray-2 ">
                                    <div class=" flex gap-2 items-center">
                                        <div class="mt-2  items-center py-2 px-1 rounded-[8px] w-[134px] flex justify-between"
                                            style="box-shadow: 1px 1px 7px rgba(0, 0, 0, 0.25);">
                ...
                <div class="w-full xl:max-w-[327px]">
                    <div class="py-[32px] px-[24px]  rounded-[10px] "
                        ...
                        <div class="my-4 flex justify-between">
                            ...
                            <p class="font-lg-responsive">Rp <span id="totalHarga">0</span></p>
                        </div>
                        <div class="mt-[40px]">
                            <a href="/product/pembayaran">
...
<script>
    $(function(){
        ...
        $(".jumlah-input").on('change',function (e) {
          let harga = $(this).parents("tr").find(".harga").attr("data-harga");
          console.log(harga);
        })

        $(".tblPlusJumlah").on('click',function (e) {
            let parent =  $(this).parents("tr");
            let harga =parent.find(".harga").attr("data-harga");
            let jumlahInput = parent.find('.jumlah-input');
            let jumlahInputValue = parseInt(jumlahInput.val());
            jumlahInput.val(parseInt(jumlahInputValue += 1));
            
            parent.find(".subtotal").html(moneyFormat.format(jumlahInputValue * harga));
            parent.find(".subtotal").data("subtotal",jumlahInputValue * harga);
            handleTotal();
        });
        ...
        $(".tblPlusHari").on('click',function() {
            let parent =  $(this).parents("tr");
            let hariInput = parent.find('.hari-input');
            let hariInputValue = parseInt(hariInput.val());
            hariInput.val(parseInt(hariInputValue += 1));
        })
        $(".tblMinHari").on('click',function() {
            let parent =  $(this).parents("tr");
            let hariInput = parent.find('.hari-input');
            let hariInputValue = parseInt(hariInput.val());
            if(hariInputValue > 0 ){
        ...
        const handleTotal = () => {
            let total = 0;
            $('.item-keranjang').each(function(i, obj) {
                let parent =$(obj);
                let subtotal = parent.find('.subtotal').data('subtotal');
                total+= parseInt(subtotal)
            });

            $("#totalHarga").html(moneyFormat.format(total));
